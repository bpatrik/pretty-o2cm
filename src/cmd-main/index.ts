...
import {PlacementParser} from '../o2cm-parser/DancerNameParser';


const run = async () => {
  try {

    console.log(JSON.stringify((await EventParser.parse('bbe17',
      DivisionTypes.Amateur,
      {type: EventSkillTypes.Newcomer, str: 'Newcommer'}, HTTPLoader
    )).map(e => e.placements.map(p => p.toJSONable()))
      .map(d => d.filter(p => p.follower.firstName.trim() === ''))));
